...
class DcChargerDemandSection(QObject):
    ...
    def __init__(self):
        ...
        self.dc_charger_demand: list[float] = ([0.0]*5) + ([32.4] * 17) + [0.0, 0.0]

        self.load_on_ess: list[float] = ([0.0]*5) + ([3.6] * 17) + [0.0, 0.0]

        self.ess_charge: list[float] = [28.8, 57.6, 86.4, 115.2, 144.0, 140.4, 136.8, 133.2, 129.6, 126.0, 122.4, 118.8,
        	                115.2, 111.6, 108.0, 104.4, 100.8, 97.2, 93.6, 90.0, 86.4, 82.8, 111.6, 140.4]

        self.ess_state_of_charge: list[float] = [0.8, 0.16, 0.24, 0.33, 0.41, 0.40, 0.39, 0.38, 0.37, 0.36, 0.35, 0.34,
...
# ================================================================
    ...
    def setEssChargeElement(self, index:int, ess_charge:float):
        if self.ess_charge[index] != ess_charge:
...
# ================================================================
    ...
    def setEssStateOfChargeElement(self, index:int, ess_state_of_charge:float):
        if self.ess_state_of_charge[index] != ess_state_of_charge:
